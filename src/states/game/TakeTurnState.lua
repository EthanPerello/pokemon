            ...
            Timer.after(1.5, function()
                ...
                :finish(function()
                    ...
                    if self.playerPokemon.currentExp > self.playerPokemon.expToLevel then
                        ...
                        local HPX = self.playerPokemon.HPX
                        local HPY = self.playerPokemon.HPIncrease
                        local HPZ = self.playerPokemon.HPZ
                        local AttackX = self.playerPokemon.AttackX
                        local AttackY = self.playerPokemon.AttackIncrease
                        local AttackZ = self.playerPokemon.AttackZ
                        local DefenseX = self.playerPokemon.DefenseX
                        local DefenseY = self.playerPokemon.DefenseIncrease
                        local DefenseZ = self.playerPokemon.DefenseZ
                        local SpeedX = self.playerPokemon.SpeedX
                        local SpeedY = self.playerPokemon.SpeedIncrease
                        local SpeedZ = self.playerPokemon.SpeedZ
                        local levelupinfo = string.format('HP: %s + %s = %s Attack: %s + %s = %s       Defense: %s + %s = %s Speed: %s + %s = %s', HPX, HPY, HPZ, AttackX, AttackY, AttackZ, DefenseX, DefenseY, DefenseZ, SpeedX, SpeedY, SpeedZ)
                        gStateStack:push(BattleMessageState(levelupinfo,
                        function()
                            self:fadeOutWhite()
                        end))
